...
class FileUploadTests(APITestCase):
    ...
    def test_file_exists(self):
        ...
        response = self.client.get(reverse('file-details', args=[self.id]))
    ...
    def test_file_does_not_exist(self):
        self.assertEqual(FilesModel.objects.count(), 1)
        response = self.client.get(reverse('file-details', args=[self.id + 1]))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)


    def test_file_gets_deleted_successfully(self):
        self.assertEqual(FilesModel.objects.count(), 1)
        response = self.client.delete(reverse('file-details', args=[self.id]))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(FilesModel.objects.count(), 0)

    def test_file_does_not_get_deleted_if_it_does_not_exist(self):
        self.assertEqual(FilesModel.objects.count(), 1)
        response = self.client.delete(reverse('file-details', args=[self.id + 1]))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(FilesModel.objects.count(), 1)